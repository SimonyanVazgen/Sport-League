...
public class start_bascketball_sr4 extends AppCompatActivity {

    private static final int IMAGE_SELECTION_REQUEST = 1;
    // Adjusted the array size to 11 since you have 11 buttons
    private boolean[] buttonImagesSet = new boolean[5]; // Tracking state for each button
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_start_bascketball_sr4);
        ...
        int[] buttonIds = new int[]{
                R.id.button12bsr4, R.id.button13bsr4, R.id.button14bsr4, R.id.button15bsr4,
                R.id.button16bsr4
        };
    ...
    private void setupButtonClickListener(int buttonId) {
        ...
        button.setOnClickListener(v -> {
            Intent intent = new Intent(start_bascketball_sr4.this, start_basketball_my_cards.class);
    ...
    private void updateButtonImageSetState(int buttonId, boolean state) {
        int index = -1;
        if (buttonId == R.id.button12bsr4) {
            index = 0;
        } else if (buttonId == R.id.button13bsr4) {
            index = 1;
        } else if (buttonId == R.id.button14bsr4) {
            index = 2;
        } else if (buttonId == R.id.button15bsr4) {
            index = 3;
        } else if (buttonId == R.id.button16bsr4) {
            index = 4;
        }
    ...
    private void checkNextButtonState() {
        Button nextButton = findViewById(R.id.next_bs4);
        boolean allSet = true;
        ...
        if (allSet) {
            nextButton.setOnClickListener(v -> {
                Intent intent = new Intent(start_bascketball_sr4.this, basketball_next.class);
